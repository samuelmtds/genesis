...
public class FormMetadataFactoryAspect {
	...
	public static class FormMetadataFactoryImpl implements FormMetadataFactory {

		private final FormMetadataFactory factory = new AspectFormMetadataFactory();

		public FormMetadata getFormMetadata(Class formClass) {
			return factory.getFormMetadata(formClass);
		}
	}

	public static class AspectFormMetadataFactory implements
			FormMetadataFactory {
		...
		private void processFormMetadata(final FormMetadata formMetadata,
			...
			processConditionAnnotation(formMetadata, Annotations
					.getAnnotations(MetadataAttribute.CONDITION.getName(),
							formClass));
				...
				for (int i = 0; i < propertyDescriptors.length; i++) {
					...
					if (!propDesc.getName().equals("class")) {
						...
						formMetadata.addFieldMetadata(propDesc.getName(),
								fieldMetadata);
						processFieldMetadata(formMetadata, fieldMetadata,
								propDesc);
					}
				}
		...
		private void processFieldMetadata(final FormMetadata formMetadata,
			...
			processEnabledWhenAnnotation(fieldMetadata,
					(UntypedAnnotationProxy) Annotations.getAnnotation(
							MetadataAttribute.ENABLED_WHEN.getName(),
							readMethod));
			processClearOnAnnotation(fieldMetadata,
					(UntypedAnnotationProxy) Annotations.getAnnotation(
							MetadataAttribute.CLEAR_ON.getName(), readMethod));
			processDisplayOnlyAnnotation(fieldMetadata, Annotations
					.getAnnotation(MetadataAttribute.DISPLAY_ONLY.getName(),
							readMethod));
			processEmptyValueAnnotation(fieldMetadata,
					(UntypedAnnotationProxy) Annotations
							.getAnnotation(MetadataAttribute.EMPTY_VALUE
									.getName(), readMethod));
			...
			processEmptyResolverAnnotation(fieldMetadata,
					(UntypedAnnotationProxy) Annotations.getAnnotation(
							MetadataAttribute.EMPTY_RESOLVER.getName(),
							readMethod));

		}

		private void processConditionAnnotation(
				final FormMetadata formMetadata, List annotations) {
			...
			for (Iterator iter = annotations.iterator(); iter.hasNext();) {
				...
				if (equalIndex > 0) {
					formMetadata.addNamedCondition(value.substring(0,
							equalIndex), JXPathContext.compile(value
		...
		private void processEnabledWhenAnnotation(
			...
			if (annotation != null) {
				fieldMetadata.setEnabledCondition(JXPathContext
						.compile(annotation.getValue()));
			}
		}

		private void processClearOnAnnotation(
				final FieldMetadata fieldMetadata,
				final UntypedAnnotationProxy annotation) {
			if (annotation != null) {
				fieldMetadata.setClearOnCondition(JXPathContext
						.compile(annotation.getValue()));
			}
		}
		...
		private void processEmptyValueAnnotation(
			...
			if (annotation != null) {
				value = converter.convert(fieldMetadata.getFieldClass(),
						annotation.getValue());
			} else {
		...
		private void processEmptyResolverAnnotation(
			...
			if (annotation != null) {
				resolver = getEmptyResolver(fieldMetadata, annotation
						.getValue());
			} else {
		...
		private Map getAttributesMap(final String attributesLine) {
			...
			while (spaceSeparator.hasMoreTokens()) {
				...
				if (equalSeparator.countTokens() == 2) {
					attributesMap.put(equalSeparator.nextToken(),
							equalSeparator.nextToken());
				}
			}
